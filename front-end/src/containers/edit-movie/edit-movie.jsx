...
const EditMoviePage = () => {
    ...
    const handleGetDataMovie = async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/admin/edit/${movieId}`);
    ...
    /* const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.patch(`${import.meta.env.VITE_API_URL}/admin/edit/${movieId}`, {
                ...state
            });
            if (response.status === 200) {
                console.log("Success !");
    ...
    return (
        <div className={cx("create-movie-page")}>
            <form className={cx("form-add-movie")} >
                <h3>Edit Movie</h3>
                <input type="date" name="created" value={state.created} onChange={(e) => handleChangeForm("created", e)} className={cx("form-input")} required />
                <input type="text" name="name" value={movieData.movie?.name} onChange={(e) => handleChangeForm("name", e)} placeholder="Name" className={cx("form-input")} required />
                <input type="text" name="slug" value={movieData.movie?.slug} onChange={(e) => handleChangeForm("slug", e)} placeholder="Slug" className={cx("form-input")} required />
                <input type="text" name="origin_name" value={movieData.movie?.origin_name} onChange={(e) => handleChangeForm("origin_name", e)} placeholder="Origin Name" className={cx("form-input")} required />
                <textarea rows="10" name="content" value={movieData.movie?.content} onChange={(e) => handleChangeForm("content", e)} placeholder="Content" className={cx("form-input")} required />
                <select name="type" value={movieData.movie?.type} onChange={(e) => handleChangeForm("type", e)} className={cx("form-select")}>
                    <option value="hoathinh">hoathinh</option>
                    <option value="series">series</option>
                    <option value="single">single</option>
                    <option value="tvshows">tvshows</option>
                </select>
                <select name="status" value={movieData.movie?.status} onChange={(e) => handleChangeForm("status", e)} className={cx("form-select")}>
                    <option value="ongoing">ongoing</option>
                    <option value="completed">completed</option>
                </select>
                <input type="text" name="poster_url" value={movieData.movie?.poster_url} onChange={(e) => handleChangeForm("poster_url", e)} placeholder="Poster" className={cx("form-input")} required />
                <input type="text" name="thumb_url" value={movieData.movie?.thumb_url} onChange={(e) => handleChangeForm("thumb_url", e)} placeholder="Thumbnail" className={cx("form-input")} required />
                <input type="text" name="trailer_url" value={movieData.movie?.trailer_url} onChange={(e) => handleChangeForm("trailer_url", e)} placeholder="Trailer" className={cx("form-input")} required />
                <input type="text" name="time" value={movieData.movie?.time} onChange={(e) => handleChangeForm("time", e)} placeholder="Time" className={cx("form-input")} />
                <input type="text" name="episode_current" value={movieData.movie?.episode_current} onChange={(e) => handleChangeForm("episode_current", e)} placeholder="Episode Current" className={cx("form-input")} required />
                <input type="text" name="episode_total" value={movieData.movie?.episode_total} onChange={(e) => handleChangeForm("episode_total", e)} placeholder="Episode Total" className={cx("form-input")} required />
                <input type="text" name="quality" value={movieData.movie?.quality} onChange={(e) => handleChangeForm("quality", e)} placeholder="Quality" className={cx("form-input")} required />
                <input type="text" name="lang" value={movieData.movie?.lang} onChange={(e) => handleChangeForm("lang", e)} placeholder="Lang" className={cx("form-input")} required />
                <select name="year" value={movieData.movie?.year} onChange={(e) => handleChangeForm("year", e)} className={cx("form-select")}>
                ...
                <input type="text" name="actor" value={movieData.movie?.actor} onChange={(e) => handleChangeForm("actor", e)} placeholder="Actor" className={cx("form-input")} required />
                <input type="text" name="director" value={movieData.movie?.director} onChange={(e) => handleChangeForm("director", e)} placeholder="Director" className={cx("form-input")} required />
                ...
                <input type="text" name="episodes" value={state.episodes} onChange={(e) => handleChangeForm("episodes", e)} placeholder="Example: tap-1|filename|link_embed|link_m3u8,..." className={cx("form-input")} required />
