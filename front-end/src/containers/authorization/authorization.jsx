...
const Authorization = () => {
    ...
    const handleLocked = async (event) => {
        ...
        try {
            const response = await axios.patch(`${import.meta.env.VITE_API_URL}/admin/users/lock`, {
                userIdList: idList,
            });
    ...
    const handleUnlocked = async (event) => {
        event.preventDefault();
        try {
            const response = await axios.patch(`${import.meta.env.VITE_API_URL}/admin/users/unlock`, {
                userIdList: idList,
            });
            if (response.status === 200) {
                console.log("Success");
            ...
            {users.map((user, index) => {
              return (
                <tr key={user._id} className={cx("tb-row")}>
                  ...
                  <td>{user.email}</td>
                  <td>{user.role}</td>
                  <td>{!user.isLocked ? "Unlock" : "Lock"}</td>
                  <td>{user.createAt}</td>
                </tr>
              );
            })}
