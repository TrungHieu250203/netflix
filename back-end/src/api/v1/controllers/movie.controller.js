...
module.exports.addMovieToMyList = async (req, res) => {
  try {
    ...
    if (myList) {
      const isMovie = Array.from(myList._doc.movie).find((movie) => {
  ...
  try {
    ...
    if (status) {
      const foundMovieStatus = status.movies.find((m) => {
        return m.movieId.equals(movie.id);
      });
      if (foundMovieStatus) {
    ...
    const score = {};
    if(totalScore) {
      score.totalScore = totalScore.totalScore;
      score.voteQuantity = totalScore.voteQuantity;
    }
    const response = {
      detail: movie,
      comments: comments,
